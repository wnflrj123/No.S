...
export default function CastingScheduleTable({ rows, displayRoleNames, inviteId }: Props) {
  ...
  const dayTone = (day: string) =>
    day === '토' ? 'text-[#1E5FA8]' : day === '일' ? 'text-[#B9304B]' : 'text-gray-800';
    ...
    <>
        ...
        <table className="w-full text-[11.5px] border-separate border-spacing-0 table-fixed">
          ...
          <thead>
            <tr>
              <th
                scope="col"
                className="pl-2 pr-1 py-2 text-left text-[9px] font-bold uppercase tracking-[0.18em] text-[#054A85] bg-[#F7EFDB]/45 border-y-2 border-double border-[#0066B3]/25"
              >
                배역
              </th>
              {rows.map((row, i) => {
                ...
                return (
                  <th
                    ...
                    className={`px-1 py-2 text-center align-middle bg-[#F7EFDB]/45 border-y-2 border-double border-[#0066B3]/25 ${
                      hasCasting
                        ? 'cursor-pointer hover:bg-[#FAF3E2]/65 focus:bg-[#FAF3E2]/75 focus:outline-none focus-visible:ring-2 focus-visible:ring-[#0066B3]/45 focus-visible:ring-inset'
                        : ''
                    }`}
                  >
                    <div className={`flex flex-col items-center gap-0.5 ${tone}`}>
                      <span
                        aria-hidden
                        className="text-[8.5px] font-bold tracking-[0.1em] text-[#0066B3]/45 tabular-nums leading-none"
                      >
                        {romanize(i + 1)}
                      </span>
            ...
            {displayRoleNames.map((name, ri) => {
              ...
              return (
                <tr key={name}>
                  <th
                    scope="row"
                    className={`pl-2 pr-1 py-2.5 text-left text-[11px] font-semibold text-[#054A85]/85 tracking-tight break-keep ${cellBorder}`}
                  >
                    {name}
                  </th>
                  {rows.map((row, i) => {
                    ...
                    return (
                      <td
                        key={i}
                        onClick={() => open(i)}
                        className={`px-1 py-2.5 text-center text-[11.5px] text-gray-800 tracking-tight break-keep ${cellBorder} ${
      ...
      <div className="hidden sm:block overflow-x-auto -mx-1 px-1">
        <table className="w-full text-[13px] border-separate border-spacing-0">
          <thead>
            <tr className="bg-[#F7EFDB]/45 [&>th]:border-y [&>th]:border-double [&>th]:border-y-2 [&>th]:border-[#0066B3]/25">
              <th
                scope="col"
                className="px-2.5 py-2.5 text-left text-[10px] font-bold uppercase tracking-[0.22em] text-[#054A85] whitespace-nowrap"
              >
                Date
              </th>
              <th
                scope="col"
                className="px-1.5 py-2.5 text-center text-[10px] font-bold tracking-wider text-[#054A85] whitespace-nowrap"
              >
                요일
              </th>
              <th
                scope="col"
                className="px-2.5 py-2.5 text-center text-[10px] font-bold uppercase tracking-[0.22em] text-[#054A85] whitespace-nowrap"
              >
                Time
              </th>
              {displayRoleNames.map(name => (
                <th
                  key={name}
                  scope="col"
                  className="px-2 py-2.5 text-center text-[12px] font-bold tracking-tight text-gray-900 break-keep whitespace-nowrap"
                >
                  {name}
                </th>
              ))}
